...
- **Auto-detect Clipboard** — Automatically detect new images in clipboard and save them
- **Return to Target Window** — Automatically return to the previous app window after paste
- **Delete File After Paste** — Auto-delete source file after successful paste
...
- **自動識別剪貼簿** — 開啟後自動偵測剪貼簿中的新圖片並儲存
- **貼上後回到目標視窗** — 貼上完成自動回到之前的應用視窗
- **貼上後刪除檔案** — 貼上成功後自動刪除來源檔案
...
- **クリップボード自動検出** — クリップボードの新しい画像を自動検出し保存
- **貼り付け後にターゲットウィンドウに戻る** — 貼り付け完了後、元のアプリに戻る
- **貼り付け後にファイルを削除** — 貼り付け成功後、ソースファイルを自動削除
...
- **클립보드 자동 감지** — 클립보드의 새 이미지를 자동 감지하여 저장
- **붙여넣기 후 대상 윈도우로 돌아가기** — 붙여넣기 완료 후 이전 앱으로 자동 돌아감
